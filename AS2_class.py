...
class Asym_slab:
    ...
    def m0_mp(self, c_ph, M_A0):
        return mp.sqrt( (1 - (c_ph - M_A0)**2) * (self.c0**2 - (c_ph - M_A0)**2) /
                      ( (self.c0**2 + 1) * (self.c_T0**2 - (c_ph - M_A0)**2)) )

    def m1_mp(self, c_ph, M_A1):
        return mp.sqrt( (self.v_A1**2 - (c_ph - M_A1)**2) * (self.c1**2 - (c_ph - M_A1)**2 ) /
                       ((self.c1**2 + self.v_A1**2) * (self.c_T1**2 - (c_ph - M_A1)**2) ) )

    def m2_mp(self, c_ph, M_A2):
        return mp.sqrt( (self.v_A2**2 - (c_ph - M_A2)**2) * (self.c2**2 - (c_ph - M_A2)**2 ) /
                       ((self.c2**2 + self.v_A2**2) * (self.c_T2**2 - (c_ph - M_A2)**2) ) )

    def disp_rel_mp(self, c_ph, kx0, v_A1, v_A2, M_A0, M_A1, M_A2):
        return (1 - (c_ph - M_A0)**2 )**2 + \
            self.m0_mp(c_ph, M_A0)**2 * self.R1 * self.R2 / (self.m1_mp(c_ph, M_A1) * self.m2_mp(c_ph, M_A2)) * \
            (v_A1**2 - (c_ph - M_A1)**2) * (v_A2**2 - (c_ph - M_A2)**2) + \
            1/2 * (1 - (c_ph - M_A0)**2) * \
            (self.m0_mp(c_ph, M_A0) * self.m1_mp(c_ph, M_A1)**(-1) * self.R1 * (v_A1**2 - (c_ph - M_A1)**2) +
            self.m0_mp(c_ph, M_A0) * self.m2_mp(c_ph, M_A2)**(-1) * self.R2 * (v_A2**2 - (c_ph - M_A2)**2)) * \
            ( mp.tanh(self.m0_mp(c_ph, M_A0) * kx0) + mp.tanh(self.m0_mp(c_ph, M_A0) * kx0)**(-1) )

class Asym_slab_corona:

    def __init__(self, R1, R2, kx0, c0, c1, c2, M_A0, M_A1, M_A2):
        ...
        self.v_A1 = sc.sqrt(2*self.R1**(-1) * (self.c0**2 + 5/6) - 2 * c1**2)
        self.v_A2 = sc.sqrt(2*self.R2**(-1) * (self.c0**2 + 5/6) - 2 * c2**2)
